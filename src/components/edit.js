...
export default function Edit(props) {
    ...
    const handleSubmit = (event) => {
        ...
        // Create a new book object with updated data from the component's state
        const newBook = {
            id: id,
            title: title,
            cover: cover,
        ...
        // Send a PUT request to update the book data on the server
        axios.put('http://localhost:4000/api/books/' + id, newBook)
        .then((res) => {
            // Log the response data from the server
            console.log(res.data);
    
            // Navigate to the '/read' route after a successful update
            navigate('/read');
        });
    }
